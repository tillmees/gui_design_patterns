...
class AppView(QMainWindow):
    ...
    def bind(self, view_model):
        # signals from viewmodel
        view_model.button_color_changed.connect(self.update_button_color)
        view_model.slider_set.connect(self.set_slider)
        view_model.slider_moved.connect(self.update_slider_value)
        view_model.text_input_set.connect(self.set_text_input)
        view_model.text_changed.connect(self.update_text_output)
        view_model.button_visibility_changed.connect(self.update_button_visibilities)
        view_model.element_list_changed.connect(self.update_element_list)

        # user interactions with gui
        self.change_color_button.clicked.connect(
            view_model.on_change_color_button_clicked
        )
        self.slider.valueChanged.connect(
            lambda: view_model.on_slider_moved(self.slider.value())
        )
        self.text_input.textChanged.connect(
            lambda: view_model.on_text_changed(self.text_input.toPlainText())
        )
        self.left_button.clicked.connect(view_model.move_button_clicked)
        self.right_button.clicked.connect(view_model.move_button_clicked)

        self.element_add_button.clicked.connect(
            lambda: view_model.on_add_element_button_clicked(
                self.element_text.toPlainText()
            )
        )
        self.element_delete_button.clicked.connect(
            lambda: view_model.on_delete_element_button_clicked(
                self.list_widget.currentRow()
            )
        )
        self.element_clear_button.clicked.connect(
            view_model.on_clear_element_button_clicked
        )
